...
case "$ACTION" in
    "")
        ...
        echo "text: "
        if [[ "$DLFREQ" -gt 0 ]] then
            busybox printf "text:Current cell: %d, B%d, %d.%dMhz" $PCI $BAND "$(($DLFREQ / 10))" "$(( $DLFREQ % 10))"
        else
    ...
    FIX_CELL)
        ...
        PARAMS="$(busybox printf 53810:03:00:00:00:%02x:01:%02x:01:%02x:%02x:%02x:%02x:00:00:00:00 "$BAND" "$BAND" "$((PCI%256))" "$((PCI/256))" "$((DLFREQ%256))" "$((DLFREQ/256))")"
